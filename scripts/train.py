...
def train_step(
    ...
    def loss_fn(
        ...
        #     # Get input/output dimensions from the weight matrix shape
        #     weight_shape = model.state_proj.kernel.shape
        #     logging.info(f"State projection weight matrix shape: {weight_shape}")
        #     logging.info(f"Input state shape: {observation.state.shape}")
    ...
    logging.info(f"Batch observation shape: {jax.tree_map(lambda x: x.shape, observation)}")
    logging.info(f"Batch actions shape: {jax.tree_map(lambda x: x.shape, actions)}")
    ...
    try:
        ...
        logging.error("Error in forward/backward pass:", exc_info=True)
        raise
...
def plot_action_scatter(gt_actions: np.ndarray, pred_actions: np.ndarray, step: int, plot_dir: str):
    ...
    # Plot first trajectory in batch
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plot_dir, f'action_scatter_plot_{step}.jpg'))
