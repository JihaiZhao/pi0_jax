...
import openpi.policies.xarm_dual_policy as xarm_dual_policy
import openpi.policies.xarm_single_policy as xarm_single_policy
...
class DataConfig:
    ...
    # Add these parameters to limit data loading
...
class LeRobotXARMDualDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        repack_transforms = _transforms.Group(
            inputs=[
                _transforms.RepackTransform({
                    ...
                    "observation_states_joint_angle_right": "observation_states_joint_angle_right",
                    "observation_states_gripper_position_right": "observation_states_gripper_position_right",
                    "observation_states_ee_pose_left": "observation_states_ee_pose_left",
                    "observation_states_ee_pose_right": "observation_states_ee_pose_right",
                    "observation_images_head": "observation_images_head",
                    "observation_images_left_wrist": "observation_images_left_wrist",
                    "observation_images_right_wrist": "observation_images_right_wrist",
                    # Add prompt to repack transform
                    "prompt": "prompt",
                })
            ]
        )
        ...
        # Create base config and update with our transforms
...
# XArm Single Config
@dataclasses.dataclass(frozen=True)
class LeRobotXARMSingleDataConfig(DataConfigFactory):
    """Config for xarm dataset."""
    default_prompt: str | None = None
    action_sequence_keys: Sequence[str] = ("action")
    max_episodes: int | None = None
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        repack_transforms = _transforms.Group(
            inputs=[
                _transforms.RepackTransform({
                    ...
                    "action": "action",
                    # Keep observation columns
                    "observation_states_joint_angle": "observation_states_joint_angle",
                    "observation_states_gripper_position": "observation_states_gripper_position",
                    "observation_images_d455": "observation_images_d455",
                    "observation_images_gopro": "observation_images_gopro",
                    # Add prompt to repack transform
                    "prompt": "prompt",
                })
            ]
        ...
        # Then do data transforms
        data_transforms = _transforms.Group(
            inputs=[xarm_single_policy.XarmInputs(action_dim=model_config.action_dim, model_type=model_config.model_type)],
            outputs=[xarm_single_policy.XarmOutputs()],
        )
...
class TrainConfig:
    ...
    # How often (in steps) to generate plots
    ...
    # Number of batches to load at once
...
def cli() -> TrainConfig:
    """command line interface for training configs"""
